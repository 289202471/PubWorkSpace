...
object LogicRegressWineClassifyDemo {
  def main(args: Array[String]): Unit = {
    ...
    //        conf.setMaster("local[4]");//这里可能会不一样，因为集群配置的问题
    //         val sc=new SparkContext(conf);
    //         val sess=new SparkSession(sc);local[4]
    //集群初始化
    val sess = SparkSession.builder().appName("ml").master("spark://s101:7077").getOrCreate();
    val sc = sess.sparkContext;

    //数据文件目录，一会儿不要忘记写
    val dataDir="hdfs://s101:8020/user/ubuntu/hadoop/sparkdata/winequality-red1.csv";
